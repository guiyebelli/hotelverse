...
import { getLimitPokemons, getPokemon } from '@/utils/fetch'
...
export async function generateStaticParams() {
  const data: PokemonsReponse = await getLimitPokemons(5)
  const static5Pokemons = data.results.map((pokemon) => ({
    name: pokemon.name,
  }))

  return static5Pokemons.map(({ name }) => ({
    name: name,
  }))
}
    ...
    <>
      <div className="flex mt-5 flex-col items-center text-slate-800">
        <Link className="flex items-center " href={'/'}>
          ...
          Volver a la lista de todos los Pokemons
        </Link>
      </div>
